                ...
                <div class="col-md-10">
                    <div class="card shadow-lg border-0 rounded-4 p-5 bg-white position-relative">
                        ...
                        <div class="text-center mt-5 pt-4">
                            ...
                            <div class="progress rounded-pill bg-light shadow-sm mx-auto"
                                style="height: 10px; max-width: 400px;">
                                <div class="progress-bar bg-primary rounded-pill transition-bar"
                        ...
                        <form class="mt-4" @submit.prevent="submitForm">
                            ...
                            <div v-if="step === 0" class="row g-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input v-model="form.telefoneConsultorio" type="text" class="form-control"
                            ...
                            <div v-if="step === 1" class="row g-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        <input :type="showPassword ? 'text' : 'password'" v-model="form.senha"
                            ...
                            <div v-if="step === 2" class="row g-4">
                                <div class="col-md-4">
                                    ...
                                    <input v-model="form.crm" type="text" class="form-control" @input="formatCRM"
                                        required />
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select v-model="form.uf" class="form-select" required>
                                        ...
                                        <option v-for="estado in ufs" :key="estado" :value="estado">{{ estado }}
                                        </option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input v-model="form.valorConsulta" type="text" class="form-control"
                                        @input="formatarValorConsulta" required />
                                    <small v-if="valorInvalido" class="text-danger">Valor deve ser maior que R$
                                        0,00</small>
                                </div>
                                <div class="col-md-6">
                            ...
                            <div v-if="step === 3">
                                ...
                                <div class="row">
                                    <div class="col-md-4 mb-3" v-for="(dia, index) in Object.entries(diasAtendimento)"
                                        :key="index">
                                        <label>{{ dia[0].charAt(0).toUpperCase() + dia[0].slice(1) }}</label>
                                        <div class="d-flex">
                                            <select v-model="dia[1].inicio" class="form-select me-2"
                                                @change="validateHorario(dia)">
                                                <option value="">Início</option>
                                                <option v-for="h in horarios" :key="h" :value="h">{{ h }}</option>
                                            </select>
                                            <select v-model="dia[1].fim" class="form-select"
                                                @change="validateHorario(dia)">
                            ...
                            <div class="d-flex justify-content-between mt-4">
                                <button v-if="step > 0" type="button" class="btn btn-outline-primary btn-lg px-4"
                                    @click="step--">←
                                    Voltar</button>
                                <button v-if="step < 3" type="button" class="btn btn-primary btn-lg ms-auto px-4"
                                    @click="proximo">Próximo →</button>
                                <button v-else type="submit" class="btn btn-success btn-lg ms-auto px-5">
...
export default {
    ...
    methods: {
        ...
        async carregarMedico() {
            try {
                const { data } = await medicoApi.get(`/${this.id}`);

                const valorFormatado = `R$ ${parseFloat(data.valorConsulta).toFixed(2).replace('.', ',')}`;
                this.form = {
                    ...
                    email: data.email,
                    senha: "",
                    telefoneConsultorio: data.telefoneConsultorio,
                    crm: data.crm,
                    uf: data.uf,
                    especialidade: data.especialidade,
                    valorConsulta: valorFormatado,
                    tempoConsulta: data.tempoConsulta?.toString()
                };
                ...
                for (const item of data.diasAtendimento) {
                    const dia = item.diaSemana;
                    const horario = item.horario;
                    if (!novoDias[dia]) continue;
                    if (!novoDias[dia].inicio || horario < novoDias[dia].inicio) {
                        novoDias[dia].inicio = horario;
                    }
                    if (!novoDias[dia].fim || horario > novoDias[dia].fim) {
                        novoDias[dia].fim = horario;
                    }
                }

                this.diasAtendimento = novoDias;
        ...
        async submitForm() {
            try {
                ...
                if (this.id) {
                    await medicoApi.put(`/update/${this.id}`, payload);
                    alert("Médico atualizado com sucesso!");
                } else {
                    await medicoApi.post('/inserir', payload);
